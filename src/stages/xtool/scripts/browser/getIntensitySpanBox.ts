...
(() => {
  const intensitySpan = [...document.querySelectorAll("span")].find(
    (s) => s.textContent.trim() === "Intensity",
  );

  if (!intensitySpan) throw new Error("failed to find intensity setting!");

  const parent =
    intensitySpan?.parentElement?.parentElement?.parentElement?.parentElement;
