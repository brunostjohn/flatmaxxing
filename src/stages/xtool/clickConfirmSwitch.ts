...
(() => {
  const confirmSwitch = [...document.querySelectorAll("button")].find(b => b.textContent?.trim() === "Switch");
  if (!confirmSwitch) {
    ...
    const m1UltraSpan = [...(spans??[])].find(s => s.textContent?.trim().includes("M1 Ultra"));
    if (m1UltraSpan) {
      return;
    }
